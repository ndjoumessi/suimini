...
export default function PersonModal({ person, tree, onClose, onUpdate, onDelete, onSelectPerson, onAddRelationship, onDeleteRelationship }: Props) {
  ...
  function addNote() {
    ...
    const notes = [...(person.notes || []), {
      id: generateId(), content: newNoteContent.trim(),
  ...
  function PersonLink({ p }: { p: Person }) {
    return (
      ...
      >
        ...
        <div style={{ flex:1 }}>
          <div style={{ fontSize:'13px', fontWeight:'700' }}>{getDisplayName(p)}</div>
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target===e.currentTarget&&onClose()}>
      <div className="modal" style={{ maxWidth:'680px' }}>
        {/* Header */}
        <div style={{ padding:'20px 24px 16px', borderBottom:'1px solid var(--border)', display:'flex', alignItems:'flex-start', gap:'16px' }}>
          ...
          <div style={{ flex:1 }}>
            <h2 className="serif" style={{ margin:'0 0 4px', fontSize:'1.4rem' }}>
              {person.firstName} {person.maidenName?`(${person.maidenName}) `:''}{person.lastName}
            </h2>
            {person.nickName && <div style={{ fontSize:'13px', color:'var(--text-muted)', marginBottom:'4px', fontStyle:'italic' }}>«&nbsp;{person.nickName}&nbsp;»</div>}
            <div style={{ display:'flex', gap:'6px', flexWrap:'wrap' }}>
              ...
              {person.occupation&&<span className="badge" style={{ background:'var(--bg-muted)', color:'var(--text-muted)' }}>{person.occupation}</span>}
              {person.nationality&&<span className="badge" style={{ background:'var(--bg-muted)', color:'var(--text-muted)' }}>🌍 {person.nationality}</span>}
            </div>
          </div>
          <button onClick={onClose} className="btn btn-ghost btn-sm" style={{ padding:'4px 8px', fontSize:'18px' }}>✕</button>
        </div>

        {/* Tabs */}
        <div className="tabs" style={{ margin:'0 24px', paddingTop:'4px' }}>
          {[
            ['profile','👤 Profil'],['family','👨‍👩‍👧 Famille'],
            ['events',`📅 Événements${person.events?.length?` (${person.events.length})`:''}` ],
            ['notes',`📝 Notes${person.notes?.length?` (${person.notes.length})`:''}` ],
            ['edit','✏️ Modifier']
          ].map(([t,label]) => (
            <button key={t} onClick={() => setTab(t as typeof tab)} className={`tab ${tab===t?'active':''}`}>{label}</button>
          ))}
        </div>
        ...
        <div style={{ padding:'16px 24px 24px', maxHeight:'60vh', overflowY:'auto' }}>
          ...
          {tab==='profile' && (
            <div className="animate-fade-in" style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
              ...
              {person.tags&&person.tags.length>0&&(
                <div>
                  <div style={{ fontSize:'11px', color:'var(--text-light)', textTransform:'uppercase', letterSpacing:'0.5px', marginBottom:'6px' }}>Tags</div>
                ...
                <div>
                  ...
                  {Object.entries(person.customFields).map(([k,v])=>(
                    <div key={k} style={{ display:'flex', gap:'8px', marginBottom:'4px', fontSize:'13px' }}>
                      <span style={{ fontWeight:'700', minWidth:'120px' }}>{k}:</span>
          ...
          {tab==='family' && (
            <div className="animate-fade-in" style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              {!showAddRel ? (
                <button onClick={()=>setShowAddRel(true)} className="btn btn-secondary btn-sm" style={{ alignSelf:'flex-start' }}>
                  ＋ Ajouter une relation
                </button>
              ) : (
                <div style={{ padding:'12px', background:'var(--bg-muted)', borderRadius:'var(--radius)' }} className="animate-fade-in">
                  <div style={{ display:'flex', gap:'8px', flexWrap:'wrap', marginBottom:'8px' }}>
                    <select value={newRelType} onChange={e=>setNewRelType(e.target.value as RelationType)} className="input" style={{ width:'auto' }}>
                    ...
                    <select value={newRelPersonId} onChange={e=>setNewRelPersonId(e.target.value)} className="input" style={{ flex:1 }}>
          ...
          {tab==='notes' && (
            <div className="animate-fade-in">
                      ...
                      {editNoteId===note.id ? (
                        <div>
                          <textarea
                            ...
                            className="input" rows={3} style={{ resize:'vertical', marginBottom:'6px', width:'100%' }}
                          />
                          <div style={{ display:'flex', gap:'6px' }}>
              ...
              {!showAddNote ? (
                ...
                <div style={{ padding:'12px', background:'var(--bg-muted)', borderRadius:'var(--radius)' }} className="animate-fade-in">
                  <textarea
                    autoFocus value={newNoteContent} onChange={e=>setNewNoteContent(e.target.value)}
                    className="input" rows={3} style={{ resize:'vertical', marginBottom:'8px', width:'100%' }}
            ...
            <div className="animate-fade-in">
              ...
              {!confirmDelete ? (
                ...
                <div style={{ padding:'12px', background:'#fdf2f2', border:'1px solid #f5c6c6', borderRadius:'var(--radius)' }}>
                  <p style={{ margin:'0 0 10px', fontSize:'13px' }}>⚠️ Supprimer définitivement <strong>{getDisplayName(person)}</strong> et toutes ses relations ?</p>
