  ...
  return (
    <div className="dash-root" style={{ flex: 1, overflowY: 'auto', background: 'radial-gradient(130% 80% at 50% -5%, rgba(201,168,76,0.06), transparent 58%), var(--bg)' }}>
      ...
      <style>{`
        ...
        /* NB: .dash-quick sits INSIDE a .dash-card (--bg-card) — the tiles need a step
           further, not the same fill as their own parent, or they'd blend into it. */
        .dash-quick-btn { display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 8px; padding: 16px 6px; min-height: 72px; cursor: pointer; background: var(--bg-muted); border: 1px solid var(--border); border-radius: var(--radius); color: var(--text-muted); font-family: var(--font-body); font-size: 12px; font-weight: 600; transition: transform var(--t-fast) var(--ease-out), box-shadow var(--t-fast), color var(--t-fast), border-color var(--t-fast), background var(--t-fast); }
        .dash-quick-btn:hover { transform: translateY(-2px); box-shadow: var(--shadow-accent); background: color-mix(in srgb, var(--accent) 10%, var(--bg-muted)); color: var(--accent-text); border-color: var(--accent); }
        ...
        .dash-ai-card { position: relative; display: flex; flex-direction: column; align-items: flex-start; gap: 5px; text-align: left; cursor: pointer; padding: 20px 22px; background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius-lg); transition: border-color var(--t-fast), background var(--t-fast), box-shadow var(--t-fast), transform var(--t-base) var(--ease-out); }
        .dash-ai-card:hover { border-color: var(--accent); background: var(--bg-muted); box-shadow: var(--shadow-accent); transform: translateY(-2px); }
        ...
        .dash-ai-card:disabled:hover { border-color: var(--border); background: var(--bg-card); box-shadow: none; transform: none; }
