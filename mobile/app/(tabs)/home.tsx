...
export default function HomeScreen() {
    ...
    return (
      <View style={[styles.flex, { backgroundColor: colors.bg, paddingTop: insets.top }]}>
        <EmptyState
          ...
          title="Aucun arbre"
          description="Connectez-vous pour retrouver vos arbres ou explorez la démo."
        />
      </View>
    );
  }
  ...
  return (
    ...
    >
      <Header eyebrow={`Bonjour`} title={firstName} subtitle={activeTree.name} />

      {/* Active tree card */}
      <View style={styles.block}>
        <Card elevated>
          <Text style={[styles.label, { color: colors.accent }]}>ARBRE ACTIF</Text>
      ...
      {stats ? (
        <View style={styles.statsGrid}>
          <Stat value={stats.totalPersons} label="Personnes" colors={colors} />
          <Stat value={stats.totalGenerations} label="Générations" colors={colors} />
          <Stat value={stats.totalAlive} label="Vivant·e·s" colors={colors} />
          <Stat value={stats.totalEvents} label="Événements" colors={colors} />
        </View>
      ) : null}

      {/* Quick access */}
      <View style={styles.block}>
        <Text style={[styles.sectionTitle, { color: colors.accent }]}>ACCÈS RAPIDE</Text>
        <View style={styles.quickRow}>
          <Quick icon={<TreePine size={22} color={colors.text} />} label="Arbre" onPress={() => router.push('/(tabs)/tree')} colors={colors} />
          <Quick icon={<Users size={22} color={colors.text} />} label="Famille" onPress={() => router.push('/(tabs)/people')} colors={colors} />
          <Quick icon={<Clock size={22} color={colors.text} />} label="Chrono" onPress={() => router.push('/(tabs)/timeline')} colors={colors} />
        </View>
      </View>
