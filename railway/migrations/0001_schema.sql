-- 0001 — porté depuis railway/schema.sql (framework de migrations versionnées,
-- Archi F5). Idempotent (create table/index if not exists, create or replace
-- function, drop trigger if exists). Pas de BEGIN/COMMIT (le runner enveloppe,
-- voir scripts/migrate-railway.mjs). Source d'origine conservée sous
-- railway/schema.sql (miroir manuel, même discipline que le double système
-- Supabase — voir CLAUDE.md « F14 »).
--
-- ⚠️ Ce schéma était DÉJÀ appliqué en prod (migration de données du 2026-07-11,
-- 71 personnes / 122 relations) AVANT que ce framework n'existe → adopter via
-- `node scripts/migrate-railway.mjs baseline` (pas de rejeu) plutôt que `up` la
-- première fois. Toute NOUVELLE migration future doit passer par `up`.
-- ============================================================================
