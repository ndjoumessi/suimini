...
### F1 — Partage par email + partage public contournent la frontière → **TOUJOURS ACTIF, inchangé**
...
### F2 — Flux d'invitation anonyme `/invite/[token]` lit Supabase, invitations écrites sur Railway → **TOUJOURS ACTIF, inchangé**
...
### F3 — « Rollback en une ligne » documenté comme instantané/sûr, sans copie inverse Railway→Supabase → **TOUJOURS ACTIF, et il s'aggrave mécaniquement**
...
**F8 · Mineur** — Canaux et routes annexes encore branchés sur les tables Supabase mortes :
...
**F11 · Mineur** — `Person` mobile (`mobile/lib/types.ts:77-114`) n'a ni `media?: Media[]` ni `photoTags?: PhotoTag[]`, présents côté web (`src/types/index.ts:98,105`). `preserveExtra` côté serveur protège ces champs d'un écrasement, mais rien ne teste cette protection pour ces champs précis, et l'en-tête du fichier mobile (« Keep in sync when the web model changes », `types.ts:1-5`) est un vœu, pas un mécanisme. *Recommandation* : test pure-logic de parité de forme (liste de clés web ⊇ mobile, avec liste d'exclusions assumées).

## 3. Couplage RLS ↔ `authz.ts` — quatre sources de vérité

**F4 · Majeur (préventif)** — La logique d'autorisation existe en 4 exemplaires sans test de parité mécanique :
...
**F5 · Majeur (latent)** — **Railway n'a toujours aucun framework de migration** : `railway/` = `schema.sql` + `realtime-notify.sql`, pas de `railway/migrations/`, pas de table de tracking, application manuelle `psql` sur l'URL unpooled — pour le schéma qui porte désormais 100% des données de production. Le framework Supabase (`supabase/migrations/0001-0017` + `scripts/migrate.mjs`) est propre mais migre un backend en fin de vie pour le data-plane. *Recommandation* : créer `railway/migrations/` en réutilisant `migrate.mjs` (paramétré sur `RAILWAY_DATABASE_URL` unpooled).
...
**F10 · Mineur** — CLAUDE.md : 305 lignes / **61 Ko** (~200 caractères par ligne — un journal de bord compressé). Ses invariants « pièges connus » restent exacts (tous ceux re-vérifiés ici l'étaient), mais sa section « état courant » a dérivé du code :
...
**F9 · Mineur** — Storage : rollback partiellement fictif. Le flag est build-time et ne re-route que les **nouveaux** uploads ; depuis `scripts/rewrite-photo-urls-to-r2.mjs`, les URLs existantes pointent vers R2 quoi qu'il arrive — retirer le flag ne restaure pas l'affichage si R2 tombe. Domaine public `pub-*.r2.dev` (URL « Development » Cloudflare) à remplacer par un domaine custom avant que ça compte.
...
| # | Finding | Sévérité | Statut vs audit précédent |
|---|---|---|---|
| F1 | Partage email + partage public écrivent/lisent le Supabase figé (`supabaseSync.ts:433-509`, `ShareModal.tsx:6`, `/arbre/[slug]`) | **Majeur** (bug prod) | **Toujours actif** |
| F2 | `/invite/[token]` anonyme lit Supabase, invitations écrites sur Railway (`invite/[token]/page.tsx:42` → `data-layer/route.ts:21` → RPC Supabase) | **Majeur** (bug prod) | **Toujours actif** |
| F3 | Rollback « une ligne » documenté comme instantané/sûr, aucune copie inverse Railway→Supabase (`railway-migration.md:12-14`, `CLAUDE.md:128`) | **Majeur** | **Toujours actif** (fenêtre de perte croissante) |
| F4 | AuthZ en 4 exemplaires sans test de parité ; `railway-store.spec.ts` jamais en CI | **Majeur** (préventif) | Inchangé |
| F5 | Railway sans framework de migrations versionnées | **Majeur** (latent) | Inchangé |
| F6 | Écriture/lecture O(arbre entier) à chaque édition — rupture à 10× | **Majeur** (horizon) | Inchangé |
| F7 | Route handlers `/api/data/*` sans tests 401/403/happy-path | Mineur | Inchangé |
| F8 | Canaux/routes annexes sur tables Supabase mortes (`subscribeComments`, `/api/export-pdf`, notify-join, send-approval-email) | Mineur | Inchangé |
| F9 | Rollback Storage partiellement fictif ; domaine `pub-*.r2.dev` en prod | Mineur | Inchangé |
| F10 | CLAUDE.md : 5 dérives état-courant vs code (palette, rate-limit, AUDIT-V4, compte de specs, promesse rollback) | Mineur | Inchangé (+1 dérive : compte de specs) |
| F11 | `Person` mobile sans `media`/`photoTags`, parité non testée | Mineur | Inchangé |
| F12 | `server-only` partiel (3 fichiers sur ~8 concernés) ; partage éclaté dans `supabaseSync.ts` (cause racine de F1) | Mineur | Partiellement amélioré (3 marqueurs posés) |
| F13 | Code mort (`fetchMembers`), commentaire périmé (`railwayStore.ts:438-440`), `sharedByName` en dur | Observation | Inchangé |
| F14 | Double système migrations Supabase, synchronisé mais manuel | Observation | Inchangé |

**Bilan : 6 Majeurs (dont 3 bugs prod actifs), 6 Mineurs, 2 Observations.**

## Recommandations priorisées

1. **Corriger F2 (invitation anonyme) en premier** — plus petit fix (une exemption d'AuthN sur une RPC par-token + routage client), plus gros impact utilisateur (onboarding collaboratif cassé à 100% pour les nouveaux invités).
2. **Corriger F1 (partage email/public)** — porter les 5 fonctions derrière le `DataStore` + endpoint public `/api/data/public/[slug]` ; réconcilier les lignes `tree_shares` écrites dans Supabase depuis le 11/07 ; en attendant, masquer l'UI concernée.
3. **Requalifier le rollback (F3)** — une passe de doc immédiate (« frein d'urgence, perte post-11/07 ») dans `railway-migration.md` + `CLAUDE.md`, puis écrire le script de copie inverse Railway→Supabase.
4. **Poser le garde-fou d'inventaire de frontière** — test pure-logic qui échoue si un fichier hors liste blanche référence une table du data-plane : il aurait attrapé F1, F2 et F8, et empêchera la prochaine fuite.
5. **Fermer le risque AuthZ (F4) + migrations Railway (F5)** — secret `RAILWAY_TEST_DATABASE_URL` en CI, test de parité des deux providers, et `railway/migrations/` réutilisant `migrate.mjs`.
