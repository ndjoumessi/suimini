...
Neutres **braise chaude** (l'heure du soir où l'on raconte les histoires de famille), la sœur nocturne de la « Canopée » mobile. **Pas de classes utilitaires Tailwind** en pratique : **styles inline + variables CSS et classes de `globals.css`** (réécrit à cette occasion — 52 fichiers touchés, **aucune logique métier** : `src/lib`, `src/hooks`, `src/i18n`, `src/app/api` intacts ; `treeLayout.ts` inchangé, seul son rendu `components/tree/*` a bougé). L'ancien « Atelier » (indigo-nuit, brutalisme zéro-radius, Fraunces/Public Sans) n'existe plus.
...
- Couleurs (dark, défaut) : `--bg #16120e` (canvas) · `--bg-card #2a231a` (**~2× plus clair que le canvas, nettement surélevé**) · `--bg-muted #372e22` (hover/zones inertes, plus clair encore) · `--surface-3 #201a13` (sidebars, toolbars, inputs) · `--ink/--text #f3ecdf` (encre papier) · `--accent` **or `#c9a84c`** (`--accent-hover #dcbb5e`, `--accent-text #d4b257` pour l'or **en texte** sur fond sombre, `--ink-on-accent #171006` pour le texte **sur** l'or — jamais blanc sur or) · `--text-muted #aa9e8c` / `--text-light #9c9081` (≥4.5:1). ⚠️ L'écart de clarté canvas↔carte a été creusé en 2 passes après le redesign (commits `b0aab15`/`01ced4a`) — ne pas ré-aplatir les surfaces.
...
- **Thème clair réel et commutable** (`d4d57c7`) : `useDarkMode()` (`light|dark|system`, persisté `suimini_theme_mode`, défaut `dark`) pose `data-theme` sur `<html>` ; `globals.css` porte un bloc `[data-theme="light"]` complet (compagnon papier-chaud, même échelle de profondeur, contrasté contre les 6 thèmes d'accent). `applyColorTheme(colorTheme, resolved)` re-dérive les nuances d'accent pour le canvas actif. ⚠️ `theme-color` (meta) ne résout pas les CSS vars → littéraux `#16120e` (dark) / `#F4F0E6` (light) dans le hook.
