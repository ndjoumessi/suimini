...
### Backend données — Railway (Phase 1) — CUTOVER PROD OWNER-ONLY LIVE
...
- **`fetchMyMemberships` — PRÊT MAIS INACTIF** : `sharing.ts` a le patron `*Direct`/`*ViaApi` + endpoint `GET /api/data/collaboration/my-memberships` + `DataStore.getMyMemberships`, gardé par **`NEXT_PUBLIC_MEMBERSHIPS_VIA_API`** (défaut OFF → lecture directe Supabase, ZÉRO changement). Résout le caveat cross-backend (tree_members écrit Railway / lu Supabase) au moment d'élargir : flag=1 + redeploy. Reste HORS PÉRIMÈTRE tant qu'inactif.
- **État (2026-07-11)** : **cutover PROD LIVE owner-only** (`suimini.vercel.app`, `DB_BACKEND=railway` + allowlist=propriétaire seul, pooled+TLS) validé (arbre + écriture). `apiPercent` Edge Config **inchangé (=0)**. **SOAK volontaire owner-only** avant d'élargir → **NE PAS monter allowlist/apiPercent ni activer le flag memberships sans feu vert explicite**. Tests : `e2e/integration/railway-store.spec.ts` (self-skip sans `RAILWAY_TEST_DATABASE_URL`).
