...
- **Écriture** (push cloud débouncé) : CRUD explicite (add/update/delete personne, relation, journal) → flush **immédiat 0 ms** (`immediateSyncRef`) ; `updateTree` en bloc garde **700 ms**. Listener `beforeunload`/`pagehide` vide un push en attente (`pendingPushRef`) avant un F5.
- **Suppression** : `deleteChildRows(table, ids)` supprime **directement et immédiatement** en base (appelé par delete{Person,Relationship,JournalEntry}), au lieu d'attendre l'étape DELETE en fin de diff-push (qu'un F5 rapide coupait → la personne réapparaissait). `syncChildTable` a 2 gardes anti-effacement : **`rows.length===0` → skip** (un cache vide ne purge jamais l'arbre) ; **local < 50 % du distant → refuse la purge**. `recordDeletedIds` (`suimini_recent_deletes`, 60 s) empêche la résurrection d'une suppression par le merge.
