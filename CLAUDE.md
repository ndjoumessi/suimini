...
src/
  ...
  lib/                 # Réorganisé en sous-dossiers thématiques (2026-07-17) — 8 fichiers
                       #   restent à plat (treeUtils.ts, sampleData.ts, themes.ts, tokens.ts,
                       #   geo.ts, relativeTime.ts, password.ts, narrativeContext.ts) :
                       #   data/       supabase.ts, supabaseSync.ts, sharing.ts, collaboration.ts,
                       #               dataClient.ts (frontière réseau + flag cookie + défaut
                       #               serveur), dataLayerConfig.ts (règle Edge Config +
                       #               bucketing rollout), authz.ts, apiAuth.ts, apiData.ts,
                       #               rpcClient.ts, dataStore.ts, railwayDb.ts, railwayStore.ts,
                       #               r2.ts, offlineStorage.ts
                       #   sync/       syncMerge.ts, conflictQueue.ts, history.ts,
                       #               realtimeEcho.ts, realtimeRelay.ts
                       #   search/     bamilekeNames.ts, fuzzySearch.ts, duplicateDetection.ts,
                       #               ocrNormalization.ts
                       #   media/      imageCompression.ts, imageCrop.ts, uploadImage.ts,
                       #               storageProvider.ts
                       #   export/     pdfTemplates.ts, gedcomParser.ts, treeLayout.ts
                       #   notifications/  emails.ts, push.ts, rateLimit.ts
...
- `lib/data/supabaseSync.ts` : mappage lignes ↔ objets, chargement/sauvegarde, partage (`shareTree`, `setTreePublic`, `loadPublicTree`).
...
- **Serveur** (`app/api/data/*`) : `getServerAuth()` (`lib/data/apiAuth.ts`) résout l'appelant par **cookie de session** OU **`Authorization: Bearer`** (mobile) ; les requêtes tournent **sous l'identité de l'appelant** → RLS en filet. AuthZ applicative miroir des RLS dans **`lib/data/authz.ts`** (prédicats `canReadTreeAsMember`/`canWriteTreeContent`/`isTreeOwner`…) + helper **`apiData.guardTreeWrite(treeId,'write'|'owner')`**. RPC forwardées via **`callRpc`** (`lib/data/rpcClient.ts`) → `/api/data/rpc/[name]` (whitelist 17 RPC).
...
- **Frontière `getDataStore(client, caller)`** (`lib/data/dataStore.ts`) : les routes `/api/data/*` ne lisent/écrivent plus via le client Supabase en direct → elles passent par un **`DataStore`**. Deux impls : **`SupabaseStore`** (passe-plat fidèle vers `supabaseSync`/`collaboration`/`sharing` = **défaut/ROLLBACK**) et **`RailwayStore`** (`lib/data/railwayStore.ts`, SQL brut `pg`, réutilise les mappers PURS de `supabaseSync` — importés/exportés). ⚠️ L'AuthZ tourne via **`store.authz`** → **MÊME backend que les données** (sinon owner-check sur le mauvais backend = 403 à tort). `guardTreeWrite/guardTreeRead` (`apiData.ts`) résolvent le store.
...
### Temps réel Railway (relais LISTEN/NOTIFY → WebSocket) — LIVE EN PRODUCTION (2026-07-16)
...
- **Non-régression** : `e2e/sync-logic.spec.ts` + `realtime-echo.spec.ts` (logique pure, faux client). Helpers purs dans `src/lib/sync/syncMerge.ts` / `src/lib/sync/realtimeEcho.ts`.
...
- **Rate limiting** des routes Anthropic + email (`narrative` 10/h, `narrative-person` 10/h, `analyze-photo` 5/h, `ocr-document` 5/h, `search` 20/h, `send-invite-email` 30/h) : `src/lib/notifications/rateLimit.ts` (`enforceRateLimit` en tête de chaque POST) → RPC SECURITY DEFINER `consume_rate_limit` (migration **`supabase/rate-limits.sql`**, table `api_rate_limits` sans policy directe). Anonyme/démo = repli mémoire par IP. **Fail-open SCOPÉ** (sécu F4, 2026-07-16) : seule la RPC ABSENTE (pré-migration) fail-open sans filet ; toute autre panne (réseau, Supabase injoignable) retombe sur le compteur mémoire par IP. 429 = `{ error: message localisé (cookie NEXT_LOCALE), code: 'rate_limit_exceeded', retryAfter }` — les UI affichent `data.error` telles quelles.
...
- Contenu HTML dans `lib/notifications/emails.ts` (Atelier, table-based, hex littéraux, fallback Georgia). Routes : `/api/send-approval` (compte activé), `/api/send-invite-email` (invitation membre), `/api/send-approval-email` (notifie le **propriétaire** quand un membre accepte — hook *best-effort* dans `acceptInvitation` de `lib/data/sharing.ts`, couvre les deux chemins d'acceptation : bouton explicite ET auto-accept post-login).
